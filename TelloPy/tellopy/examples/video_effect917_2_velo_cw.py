...
def tracking(drone,d,dx,dy,vx,vy,S,S0):
    ...
    if d < -10:
	print('forward')
        drone.set_pitch(- gain_bf*d*2)   
        
    if vx <= cw_thre and m.fabs(dx) < 30:
	print("cw")
	...
	#drone.counter_clockwise(- gain_cw*dx)
        ...
        if dx > 150:
	    print('right')
            drone.right(gain_x_2*dx*dx + gain_vx*vx/2.0)
	
        if dx < -150:
	    print('left')
            drone.left(gain_x_2*dx*dx - gain_vx*vx/2.0)    


    if dy > 5:
	print('down')
        drone.down(gain_y_2*dy*dy + gain_vy*vy/2.0)    

    if dy < -5:
	print('up')
        drone.up(gain_y_2*dy*dy + gain_vy*vy/2.0)    
