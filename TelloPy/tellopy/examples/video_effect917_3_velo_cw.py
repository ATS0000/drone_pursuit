...
def tracking(drone,d,dx,dy,vx,vy,S,S0):
    ...
    if d < -15:
	print('forward')
        drone.set_pitch(- gain_bf*d*2.0)   
        
    if vx <= cw_thre and m.fabs(dx) < 20:
	print("cw")
	drone.clockwise(gain_cw*dx)
	#drone.counter_clockwise(- gain_cw*dx)


    else:
        if 5 < dx <= 150:
	    print('right')
            drone.right(gain_x_1*dx + m.fabs(gain_vx*vx))

        if -150 <= dx < -5:
	    print('left')
            drone.left(- gain_x_1*dx + m.fabs(gain_vx*vx))

        
        if dx > 150:
	    print('right')
            drone.right(gain_x_2*dx*dx + m.fabs(gain_vx*vx/2.0))
	
        if dx < -150:
	    print('left')
            drone.left(- gain_x_2*dx*dx + m.fabs(gain_vx*vx/2.0))    


    if 5 < dy <= 80:
	print('down')
        drone.down(gain_y_1*dy + m.fabs(gain_vy*vy))    

    if -80 <= dy < -5:
	print('up')
        drone.up(- gain_y_1*dy + m.fabs(gain_vy*vy))    


    if 80 < dy:
	print('down')
        drone.down(gain_y_2*dy*dy + m.fabs(gain_vy*vy))    

    if dy < -80:
	print('up')
        drone.up(- gain_y_2*dy*dy + m.fabs(gain_vy*vy))
