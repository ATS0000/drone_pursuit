...
# Draw bounding box
		    ...
		    ret, frame = cap.read() #cmsf
		    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV) #cmsf
		    dst = cv2.calcBackProject([hsv],[0],roi_hist,[0,180],1) #cmsf

		    rect, track_window = cv2.CamShift(dst, track_window, term_crit) #cmsf
		    (x,y,w,h) = trackwindow #cmsf
